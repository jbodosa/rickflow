...
class TrajectoryFileIterator:
    def __init__(
        ...
        num_frames_per_trajectory=None,
        infer_time=True,
        load_function=md.load
    ):
        # create topology
        if type(filenames) is list:
            self.filenames = dict(enumerate(filenames))
        else:
        ...
        self.load_function = self.interpret_load_function(next(iter(self.filenames.values())), load_function)

    def __iter__(self):
        for i, file in self.filenames.items():
            trajectory = self.load_function(file,
                                            top=self.topology,
    ...
    def __getitem__(self, index):
        trajectory = self.load_function(self.filenames[index],
                                        top=self.topology,
                                        atom_indices=self.selection)
        trajectory.i = index
...
class TrajectoryIterator(TrajectoryFileIterator):
    ...
    def __init__(self, first_sequence=None, last_sequence=None,
        ...
        if filename_template.count('{') == 1 and filename_template.count('}') == 1:
            ...
            filenames = {i: filename_template.format(i) for i in sequence_ids if os.path.exists(filename_template.format(i))}
            ...
            assert os.path.isfile(filename_template)
            self.first = 1
            self.last = 1
            filenames = {1: filename_template}
...
class CharmmTrajectoryIterator(TrajectoryIterator):
    """Old name for the TrajectoryIterator."""
