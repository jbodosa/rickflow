...
"""
THIS FILE IS FROM OPENMMTOOLS. WE PUT IT HERE TO MAKE SURE A RECENT BUGFIX IN
THE LANGEVIN INTEGRATOR IS APPLIED CORRECTLY. IT CAN BE DELETED, ONCE THE
OPENMMTOOLS VERSION BECOMES MAJOR:
...
class PrettyPrintableIntegrator(object):
    ...
    def pretty_format(self, as_list=False, step_types_to_highlight=None):
        ...
        for step in range(self.getNumComputations()):
            ...
            highlight = True if (step_types_to_highlight is not None) and (
                        step_type in step_types_to_highlight) else False
            if step_type in [8]:
                indent_level -= 1
            if highlight:
                line += '\x1b[6;30;42m'
            line += 'step {:6d} : '.format(step) + '   ' * indent_level + \
...
class ThermostatedIntegrator(utils.RestorableOpenMMObject,
    ...
    def restore_interface(cls, integrator):
        ...
        """
        restored = super(ThermostatedIntegrator, cls).restore_interface(
            integrator)
        ...
        if not restored:
            try:
                ...
                if not hasattr(integrator, 'getTemperature'):
                    logger.warning(
                        "The integrator {} has a global variable 'kT' variable "
...
class GradientDescentMinimizationIntegrator(mm.CustomIntegrator):
    ...
    def __init__(self, initial_step_size=0.01 * unit.angstroms):
        ...
        self.addComputeGlobal("accept",
                              "step(-delta_energy) * delta(energy - energy_new)")

        self.addComputePerDof("x", "accept*x + (1-accept)*x_old")

        # Update step size.
        self.addComputeGlobal("step_size",
...
class AndersenVelocityVerletIntegrator(ThermostatedIntegrator):
    ...
    def __init__(self, temperature=298 * unit.kelvin,
                 collision_rate=91.0 / unit.picoseconds,
                 timestep=1.0 * unit.femtoseconds):
        ...
        """
        super(AndersenVelocityVerletIntegrator, self).__init__(temperature,
                                                               timestep)
        ...
        self.addPerDofVariable("sigma_v",
                               0)  # velocity distribution stddev for Maxwell-Boltzmann (computed later)
        self.addPerDofVariable("collision",
...
class NoseHooverChainVelocityVerletIntegrator(ThermostatedIntegrator):
    ...
    YSWeights = {
        ...
        5: [0.2967324292201065, 0.2967324292201065, -0.1869297168804260,
            0.2967324292201065, 0.2967324292201065]
    }

    def __init__(self, system=None, temperature=298 * unit.kelvin,
                 collision_frequency=50 / unit.picoseconds,
                 timestep=0.001 * unit.picoseconds, chain_length=5, num_mts=5,
                 num_yoshidasuzuki=5):
        ...
        """
        super(NoseHooverChainVelocityVerletIntegrator, self).__init__(
        ...
        if chain_length == 0:
            print(
                "WARNING: Nosé-Hoover chain length is 0; falling back to regular velocity verlet algorithm.")
        self.M = chain_length
        ...
        if system is None:
            print(
                "SEVERE WARNING: The system was not passed to the NoseHooverChainVelocityVerletIntegrator. "
        ...
        self.addGlobalVariable("bathPE",
                               0.0)  # Thermostat bath potential energy
        ...
        for i in range(self.M):
            ...
            self.addGlobalVariable("vxi{}".format(i),
                                   0)  # Thermostat particle velocities in ps^-1
            self.addGlobalVariable("G{}".format(i),
                                   -frequency ** 2)  # Forces on thermostat particles in ps^-2
            self.addGlobalVariable("Q{}".format(i),
                                   0)  # Thermostat "masses" in ps^2 kJ/mol
    ...
    def propagateNHC(self):
        ...
        for ncval in range(self.n_c):
            for nysval in range(self.n_ys):
                self.addComputeGlobal("wdt",
                                      "w{}*dt/{}".format(nysval, self.n_c))
                self.addComputeGlobal("vxi{}".format(self.M - 1),
                                      "vxi{} + 0.25*wdt*G{}".format(self.M - 1,
                                                                    self.M - 1))
                for j in range(self.M - 2, -1, -1):
                    self.addComputeGlobal("aa",
                                          "exp(-0.125*wdt*vxi{})".format(j + 1))
                    self.addComputeGlobal("vxi{}".format(j),
                                          "aa*(aa*vxi{} + 0.25*wdt*G{})".format(
                                              j, j))
                ...
                for j in range(self.M):
                    self.addComputeGlobal("xi{}".format(j),
                                          "xi{} + 0.5*wdt*vxi{}".format(j, j))
                # update the forces
                ...
                for j in range(self.M - 1):
                    ...
                    self.addComputeGlobal("vxi{}".format(j),
                                          "aa*(aa*vxi{} + 0.25*wdt*G{})".format(
                                              j, j))
                    self.addComputeGlobal("G{}".format(j + 1),
                                          "(Q{}*vxi{}*vxi{} - kT)/Q{}".format(j,
                                                                              j,
                                                                              j,
                                                                              j + 1))
                self.addComputeGlobal("vxi{}".format(self.M - 1),
    ...
    def computeEnergies(self):
        ...
        for i in range(self.M):
            self.addComputeGlobal("bathKE",
                                  "bathKE + 0.5*Q{}*vxi{}^2".format(i, i))
...
class MetropolisMonteCarloIntegrator(ThermostatedIntegrator):
    ...
    def __init__(self, temperature=298.0 * unit.kelvin,
                 sigma=0.1 * unit.angstroms, timestep=1 * unit.femtoseconds):
        """
        ...
        super(MetropolisMonteCarloIntegrator, self).__init__(temperature,
                                                             timestep)
...
class HMCIntegrator(ThermostatedIntegrator):
    ...
    def __init__(self, temperature=298.0 * unit.kelvin, nsteps=10,
...
class LangevinIntegrator(ThermostatedIntegrator):
    ...
    def get_shadow_work(self, dimensionless=False):
        ...
        return self._get_energy_with_units("shadow_work",
    ...
    def _add_V_step(self, force_group="0"):
        ...
        # update velocities
        if self._mts:
            self.addComputePerDof("v", "v + ((dt / {}) * f{} / m)".format(
    ...
    def _parse_splitting_string(self, splitting_string):
        ...
        if mts:
            force_group_n_V = {force_group: 0 for force_group in
                               force_group_set}
            for step in step_list:
                if step[0] == "V":
...
class NonequilibriumLangevinIntegrator(LangevinIntegrator):
    ...
    def get_protocol_work(self, dimensionless=False):
        ...
        work : unit.Quantity or float
           ...
           Otherwise, the unit-bearing protocol work in units of energy.
        """
        return self._get_energy_with_units("protocol_work",
                                           dimensionless=dimensionless)
    ...
    def get_total_work(self, dimensionless=False):
        ...
        """
        return self.get_protocol_work(
            dimensionless=dimensionless) + self.get_shadow_work(
            dimensionless=dimensionless)
...
class AlchemicalNonequilibriumLangevinIntegrator(
    ...
    def __init__(self,
        ...
        self._system_parameters = {system_parameter for system_parameter in
                                   alchemical_functions.keys()}

        # call the base class constructor
        kwargs['splitting'] = splitting
        super(AlchemicalNonequilibriumLangevinIntegrator, self).__init__(*args,
                                                                         **kwargs)
    ...
    def _add_global_variables(self):
        ...
        self.addGlobalVariable('nsteps',
                               self._n_steps_neq)  # total number of NCMC steps to perform; this SHOULD NOT BE CHANGED during the protocol
        self.addGlobalVariable('step',
                               0)  # step counter for handling initialization and terminating integration
        ...
        self.addGlobalVariable('n_lambda_steps',
                               self._n_lambda_steps)  # total number of NCMC steps to perform; this SHOULD NOT BE CHANGED during the protocol
    ...
    def _add_update_alchemical_parameters_step(self):
        ...
        for context_parameter in self._alchemical_functions:
            if context_parameter in self._system_parameters:
                self.addComputeGlobal(context_parameter,
                                      self._alchemical_functions[
                                          context_parameter])
...
class ExternalPerturbationLangevinIntegrator(NonequilibriumLangevinIntegrator):
    ...
    def __init__(self, *args, **kwargs):
        super(ExternalPerturbationLangevinIntegrator, self).__init__(*args,
                                                                     **kwargs)
    ...
    def _add_global_variables(self):
        super(ExternalPerturbationLangevinIntegrator,
              self)._add_global_variables()
